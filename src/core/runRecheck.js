...
const test = () => {  
  ...
  Promise.all
  ([ runRetryWhen({maxRetryCount: 2, predicate : () => false, computation : saveCount})
    .then(runCount => console.assert(runCount == 0, `recheck min failed: ${runCount} should be 1`))

  , runRetryWhen({maxRetryCount : 2, predicate : () => true, computation : saveCount })
    .then(runCount => console.assert(runCount == 2, `recheck max failed: ${runCount} should be 2`))

  , runRetryWhen({maxRetryCount : 3, predicate : (n) => n === 0, computation : saveCount })
